...
定向回归至少覆盖未知读取范围、敏感/私有记忆隔离、统一治理写入、来源分类、恶意原始资料作为不可信数据，以及回答引用只来自真实 `MemorySource`。V2.5.1 还要求验证维护熔断、恢复 Shadow、30 天回滚、来源覆盖和事件清理安全率。完整评测映射与命令见 [LLM 治理定向评测](llm-governance-evaluation.md)。
